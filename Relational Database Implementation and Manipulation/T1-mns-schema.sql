...
COMMENT ON COLUMN appointment.appt_prior_apptno IS
    'Prior appointment number which leads to this appointment (unique one)';
...
ALTER TABLE appointment
    ADD CONSTRAINT chk_appt_length CHECK ( appt_length IN ( 'S', 'T', 'L' ) );
...
ALTER TABLE patient
    ADD CONSTRAINT chk_patient_state CHECK ( patient_state IN ( 'NT', 'QLD', 'NSW', 'ACT'
    , 'VIC',
                                                                'TAS', 'SA', 'WA' ) )
                                                                ;
...
ALTER TABLE appointment
    ADD CONSTRAINT appt_appt_fk FOREIGN KEY ( appt_prior_apptno )
